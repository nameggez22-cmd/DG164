...
**คำตอบ: เพราะ hp ไม่ได้ทำอะไรกับ main เพราะ คนละตัวแปรกับ safehp**

### 2.2 `lerp` — Local Scope

**คำถาม:** ทำไม `main()` ถึงมองไม่เห็นตัวแปร `result` ทั้งที่มันถูกประกาศไว้ในไฟล์เดียวกัน?

**คำตอบ:ไม่ได้ขี้นกับว่าไฟล์เดียวกัน แต่เป็น {}ไหน** 
...
**คำตอบ:ที่แสดงคือdicemin ของ main() แต่ที่คิดคือ min ในฟังชั่นที่มาจาก dicemin**
...
**คำตอบ: current เป็นของ ฟังชั่น ที่เป็นค่าส่งเข้ามา ไม่ใช่ตัวแปรในmain() จึงทำให้การเปลี่ยนค่าใน current ในฟังชั่น ไม่มีผลต่อ scoreที่เก็บใน main**
